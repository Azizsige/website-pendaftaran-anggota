...
export const IdCardPreview = () => {
  const { formData, frontBgPreview, backBgPreview, signaturePreview } = useIdCardSettingStore();
  const { formData: generalData, logoPreview } = useGeneralSettingStore();
  ...
  const validityDate = addMonths(dummyData.joinDate, parseInt(formData.kta_validity_months) || 12);
  ...
  return (
    <div className="flex flex-col gap-4 mt-6">
      <style>{styles}</style>
      <h3 className="text-base font-semibold text-on-surface">Live Preview</h3>
      
      <div className="flex flex-wrap gap-8 justify-center lg:justify-start">
        {/* Sisi Depan */}
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between w-[247px] mb-1 px-1">
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#F59E0B]"></span>
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-700">Sisi Depan</span>
            </div>
            {formData.kta_show_member_id === 'true' && (
              <span className="text-[9px] font-mono text-slate-500">ID: {formData.kta_member_prefix}0001</span>
            )}
          </div>
          
          {/* Wrapper untuk scaling agar pas di layar tanpa merusak layout asli */}
          <div style={{ width: '247px', height: '393px' }} className="relative">
            <div className="transform scale-[0.65] origin-top-left">
              
              <div className="relative w-[380px] h-[605px] bg-white rounded-3xl overflow-hidden card-shadow border border-slate-200/80 flex flex-col justify-between security-pattern select-none">
                {frontBgPreview && (
                ...
                <div className="relative bg-gradient-to-r from-[#07162c] via-[#0b1e3f] to-[#0f2b5c] text-white pt-6 pb-4 px-6 rounded-b-[28px] shadow-md border-b-2 border-[#F59E0B] z-20">
                  ...
                  <div className="flex items-center gap-3.5 relative z-10">
                    <div className="w-14 h-14 p-1 bg-white/10 rounded-xl backdrop-blur-sm border border-white/20 flex-shrink-0 shadow-inner flex items-center justify-center">
                      {logoPreview ? (
                        <img alt="Logo" className="w-12 h-12 object-contain drop-shadow" src={logoPreview} />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] tracking-wider text-slate-200 uppercase font-medium">Universitas Sriwijaya</span>
                      </div>
                ...
                <div className="flex-1 px-6 py-3 flex flex-col justify-between z-10">
                  <div className="flex gap-4 items-center pt-1">
                    <div className="relative flex-shrink-0">
                      {formData.kta_show_photo === 'true' ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {formData.kta_show_name === 'true' && (
                        <h3 
                          className={`font-extrabold text-[#07162c] tracking-tight mt-1 leading-snug truncate ${dummyData.name.length > 18 ? 'text-sm' : 'text-base'}`}
                          title={dummyData.name}
                        >
                          {dummyData.name}
                        </h3>
                      )}
                      {formData.kta_show_major === 'true' && (
                        <p 
                          className={`font-semibold text-[#153a7a] mt-0.5 truncate ${dummyData.major.length > 22 ? 'text-[10px]' : 'text-xs'}`}
                          title={dummyData.major}
                        >
                          {dummyData.major}
                        </p>
                      )}
                      <div className="mt-2.5 pt-2 border-t border-slate-200/80 space-y-1">
                        {formData.kta_show_nim === 'true' && (
                          <div className="flex justify-between items-center text-[10px]">
                            <span className="text-slate-500 font-medium">NIM</span>
                            <span className="font-mono font-bold text-slate-800">{dummyData.nim}</span>
                          </div>
                        )}
                        {formData.kta_show_faculty === 'true' && (
                          <div className="flex justify-between items-center text-[10px]">
                            <span className="text-slate-500 font-medium">Fakultas</span>
                            <span className="font-bold text-slate-700">{dummyData.faculty}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-50/90 rounded-2xl p-3 border border-slate-200/80 shadow-sm space-y-2 mt-2">
                    <div className="grid grid-cols-2 gap-2 text-left">
                      <div>
                        ...
                        <span className="font-mono font-extrabold text-xs text-[#0b1e3f] tracking-tight">
                          {formData.kta_show_member_id === 'true' ? `${formData.kta_member_prefix}0001` : '-'}
                        </span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-bold tracking-wider text-slate-600 uppercase">Berlaku Hingga</span>
                        <span className="font-bold text-xs text-slate-800">
                          {formData.kta_show_validity === 'true' ? format(validityDate, 'dd MMM yyyy') : '-'}
                        </span>
                      </div>
                    </div>
                    <div className="pt-1.5 border-t border-slate-200/70 grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-[10px] text-slate-700 uppercase line-clamp-1">Kominfo & Media</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-3 pt-1">
                    {formData.kta_show_qr === 'true' ? (
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="text-left">
                          <span className="inline-flex items-center gap-1 text-[8px] font-bold text-[#0f2b5c] bg-blue-50 px-1.5 py-0.5 rounded border border-blue-200">
                          ...
                          <p className="text-[9px] font-medium text-slate-700 leading-tight mt-1 max-w-[130px]">
                    ...
                    <div className="text-right flex flex-col items-end">
                      ...
                      {signaturePreview ? (
                        ...
                        <svg className="w-20 h-7 text-[#0b1e3f] -my-0.5 opacity-80" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="2" viewBox="0 0 120 40">
                          <path d="M 10 25 C 25 10, 30 35, 45 15 C 55 5, 60 30, 75 18 C 85 10, 95 28, 110 20"></path>
                          <path d="M 40 28 L 85 28" strokeDasharray="2 2" strokeWidth="1.5"></path>
                        </svg>
                      )}
                      <span className="text-[9px] font-extrabold text-slate-800">{formData.kta_signature_name || 'Nama Penanda Tangan'}</span>
                      <span className="text-[7.5px] font-mono text-slate-600">NIM 2024098112</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="relative w-[380px] h-[605px] bg-white rounded-3xl overflow-hidden card-shadow border border-slate-200/80 flex flex-col justify-between security-pattern select-none">
                ...
                <div className="relative bg-gradient-to-r from-[#07162c] via-[#0b1e3f] to-[#0f2b5c] text-white pt-6 pb-4 px-6 rounded-b-[28px] shadow-md border-b-2 border-[#F59E0B] z-20">
                  <div className="flex items-center justify-between relative z-10">
                    ...
                    <div className="text-right flex-shrink-0">
                      <span className="text-[8px] font-mono uppercase bg-white/15 px-2 py-0.5 rounded text-white tracking-widest border border-white/20 font-bold whitespace-nowrap inline-block">
                        KTA ELEKTRONIK
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex-1 px-6 pt-3 pb-5 flex flex-col justify-between z-10">
                  ...
                  <div className="bg-gradient-to-br from-slate-900 via-[#07162c] to-[#0b1e3f] rounded-2xl p-3.5 text-white shadow-sm border border-slate-700 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          ...
                          <h5 className="text-[11px] font-bold text-white tracking-tight">Sekretariat BEM FT UNSRI</h5>
                        </div>
                        <span className="text-[7.5px] font-mono text-slate-300 bg-white/10 px-2 py-0.5 rounded border border-white/10">Indralaya & Palembang</span>
                      </div>
                      <div className="space-y-1.5 text-[9px] text-slate-200 mt-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="truncate">{generalData?.org_address || 'Jl. Raya Prabumulih KM. 32, Ogan Ilir'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-4 h-4 rounded bg-white/10 flex items-center justify-center flex-shrink-0 text-amber-300">
                          ...
                          <span className="font-mono text-[8.5px] text-slate-100 truncate">{generalData?.org_email || 'bemftunsri@example.com'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-4 h-4 rounded bg-white/10 flex items-center justify-center flex-shrink-0 text-amber-300">
                          ...
                          <span className="font-mono text-[8.5px] text-amber-300 truncate">{generalData?.org_instagram || '@bemftunsri'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
